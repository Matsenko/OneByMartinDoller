...
{
	public class DwgProccesingService20 : IDwgProccessingService
	{
	...
	Dictionary<DGWViewModel, List<LwPolyline.Vertex>> roomVertices)
		...
		public List<DGWViewModel> ParseDGW(CadDocument doc)
		{
			...
			if (circLayout.ContainsKey(ObjectType.LWPOLYLINE))
			{
				...
				var linesFromTriangel = circLayout[ObjectType.LWPOLYLINE].Except(rectangles);

				foreach (var cLayout in linesFromTriangel)
				{
					LwPolyline? poliLine = cLayout as LwPolyline;
					if (poliLine == null)
					{
						continue;
					}
					var vertices = poliLine.Vertices.ToArray();
					var subLines = new List<Line>();

					for (int i = 0; i < vertices.Length-1; i++)
					{
						// Step 4: Create a line for each consecutive pair of vertices
						var startPoint = vertices[i].Location; // Use `Location` for vertex position
						var endPoint = vertices[i + 1].Location;
				...
				//lines = LinsGroupingBytConnecting(subLines);
			}
			//Lines 33 in DGW but real 32

			//17 need 32
			//ключ это квадратик, значения это линии 
			var squarLines = GetCuirtisWithRelations(lines, rectangles);
			var founded = squarLines.Keys.ToList();
			var notFounded = rectangles.Except(founded).ToList();
			var testing = GetCuirtisWithRelations(lines, notFounded.Take(3).ToList());
		...
		/// <summary>
		/// Проверяем или List<Vertex> это прямоугольник от которого идет линия
		/// </summary>
		/// <returns></returns>
		private static bool VerticalIsRectangle(List<ACadSharp.Entities.LwPolyline.Vertex> vertices)
		{
			if (vertices.Count != 4)
				return false;
			bool result = true;
			var xs = vertices.Select(x => x.Location.X).Distinct().Count() == 2;
			var ys = vertices.Select(x => x.Location.Y).Distinct().Count() == 2;
			result = xs & ys;
			return result;
		}
		...
		Dictionary<string, Dictionary<ObjectType, List<Entity>>> layEntiTypeEntity, List<Line> allLines, string mainBlockName)
		{
			...
			//может быть такое что прийдет не первая линия, которая к квадратику подсоедененна
			var lineByCount = lines.Select(l => new
			{
				line = l,
				Points = insertsForLed
				.Where(x => DoesPointConnectedToLine(x.InsertPoint, l, 10))
			})
					.ToList();
		...
		public List<List<Line>> LinsGroupingBytConnecting(List<Line> lines)
		{
		...
		public Dictionary<LwPolyline, List<Line>> GetCuirtisWithRelations(List<List<Line>> lines, List<LwPolyline> rectangles)
		{
			...
			foreach (LwPolyline rect in rectangles)
			{
				foreach (var line in lines)
				{
					...
					{
						var b1 = IsPointInPolyline(l.StartPoint, rect, 20);
						var b2 = IsPointInPolyline(l.EndPoint, rect, 20);
						if (b1 || b2)
						{
							if (result.ContainsKey(rect))
							{
								result[rect].Add(l);
							}
							else
		...
		public Dictionary<LwPolyline, Circuit> FillCuirc(List<LwPolyline> circRectangle, Dictionary<string, Dictionary<ObjectType, List<Entity>>> layouts)
		{
			...
			if (!layouts.ContainsKey("E-LUM-SP"))
			{
				throw new KeyNotFoundException("The given key 'P-BLOCK' was not present in the dictionary.");
			}
			var pBlocks = layouts["E-LUM-SP"][ObjectType.MTEXT].OfType<MText>().ToList();
